...
可以发现 $k=3$ 时算法的表现最优。这是因为一般情况下来说，语言的组合结构问题构成的词语个数都是有限的，一般都在3，4个左右。在 $k=2$ 时，无法覆盖大部分的次序颠倒的场景；在 $k$大于3时，算法的各项指标都开始逐渐下滑。这可能是因为在 $k$ 过大后，截取到的句子长度远大于次序颠倒部分的长度，使得神经网络捕捉到的信息中更多的包含了序列信息，次序颠倒信息的比重下降。
